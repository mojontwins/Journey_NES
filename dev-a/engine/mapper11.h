...
void m11_change_raw (unsigned char raw) {
	// Specially to change CHR for GNROM type mappers with incomplete bus conflict tables
	COMM_REG = raw;
	__asm__ ("jmp _change_reg");	
}

void m11_switch_raw (unsigned char raw) {
	// Changes to a CHR/PRG combination and resets
	COMM_REG = raw;
	__asm__ ("jmp _change_rom");
}
