' Make 113
' Pastes together 8 32K PRG-ROMs and 16 8K CHR-ROMs
...
Sub usage
	Print "bld11 out.nes horz|vert #prg #chr"
	Print "will read prg0-F.bin and chr0-F.bin"
End Sub
...
prg = Val (Command (3)): If prg = 0 Then prg = 16
chrn = Val (Command (4)): If chrn = 0 Then chrn = 16

? "PRG size " & (prg*2*16) & ", CHR size " & (chrn * 8)
...
If Command (2) = "vert" Then d = d Or 1
Mid (s, 7, 1) = Chr (d)
...
For j = 1 To chrn
	...
	If Lof (2) = 8192 Then
		...
		Close #2
	Else
		Print fname & " missing or not 8K. Zero-filling." & Lof(2)
		If Lof (2) = 0 Then
			Close #2
			Kill fname
		Else
