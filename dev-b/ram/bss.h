...
// Propellers
	...
	unsigned char propellers_ctr [PROPELLERS_MAX];
	unsigned char propellers_on [PROPELLERS_MAX];
	unsigned char propellers_rda [PROPELLERS_MAX];
...
unsigned char egg;
unsigned char particles_x [MAX_PARTICLES], particles_y [MAX_PARTICLES], particles_t [MAX_PARTICLES];

unsigned char prev_pant, entry, seq;
unsigned char custom_on_off;
unsigned char do_initial_dialogue;
unsigned char mission_done; 
unsigned char relay_not_activated_yet;

unsigned char lightning;

// Custom dialogues

unsigned char *gp_dia;
unsigned char pal_cycle [3];

// debug

unsigned char flicked;
