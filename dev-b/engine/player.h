...
// CUSTOM {
void player_bubble_create (void) {
	rdx = prx; rdy = pry - 4;
	rda = 1;
	particles_create ();
}
...
// CUSTOM {
void player_border (void) {
	if (!pflickering || half_life) {
		oam_index = oam_meta_spr (prx, pry + SPRITE_ADJUST, oam_index, spr_player [CELL_FACING_LEFT + CELL_FACING_LEFT + psprid]);	
	}
}
// } END_OF_CUSTOM
	...
	if (pad & PAD_A) {
		if (!pjb) {
			...
			if (pwater) {
				...
				pwaterthrustct ++;
				if (rand8 () & 0xf) player_bubble_create ();
			} else
#endif			
			if (!pj) {
				if (
...
*/
	...
	*/
	...
	if ((ppossee || pgotten) && pvy >= 0) {
		...
		if (ABS (pvx) > PLAYER_VX_MIN) {
			psprid = CELL_SWIMMING_BASE + ((prx >> 4) & 3);
		} else psprid = CELL_SWIMMING_BASE + (pwaterthrustct & 3);
	} else if (pbutt) {
		psprid = (pvy >= 0) ? CELL_BUTT_BASE + 1 : CELL_BUTT_BASE;
	} else if (pvy < 0) {
