...
void cls (void) { vram_adr (NAMETABLE_A); vram_fill (0x3f, 0x3c0); vram_fill (0x00, 0x40); }
...
void advance_worm (void) {
	...
	switch (rdt) {
		...
		case PROPELLERS_CREATE_FROM_MAP: 
			if (custom_on_off & FLAG_PROPELLERS) {
				propellers_add ();
			}
			break;
#endif
		...
		case ESTRUJATORS_TILE+2:
			if (custom_on_off & FLAG_ESTRUJATORS) {
				estrujators_add ();
		...
		case CATARACTS_TILE:
			if (custom_on_off & FLAG_CATARACTS) {
				cataracts_add ();
				cataracts_on = 1;
