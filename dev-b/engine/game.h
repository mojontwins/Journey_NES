...
void game_init (void) {
	...
	entry = prev_pant = 0xff;

	switch (level) {
		case 0:	
		case 3:
			custom_on_off = FLAG_ESTRUJATORS | FLAG_CATARACTS | FLAG_PAL_CYCLE;
			break;
		case 1:
			custom_on_off = FLAG_RANDOM_CONNECTION | FLAG_PROPELLERS | FLAG_SLOW_FANTYS;
			break;
		case 2:
			custom_on_off = FLAG_CATARACTS | FLAG_PAL_CYCLE;
			break;
	}
	...
	while (do_game) {
		...
		*/
		...
		if (!paused) {
			...
			player_border ();
			particles_move ();
			if (pwater && 0 == (frame_counter & 0x3f)) player_bubble_create ();
			
			if (lightning) {
				if (lightning == 16 || lightning == 8) pal_bg (pallightning);
				if (lightning == 12 || lightning == 4) pal_bg (l_pal_bgs [level]);
				lightning --;
			} else if (custom_on_off & FLAG_PAL_CYCLE) {
